...
typedef struct _SpoofParams_ {
  char ip[IP_SIZE];                       // 32 bit address
  char request[REQUEST_SIZE];              // website address
  char interface[PCAP_INTERFACENAME_SIZE]; // interface
} SpoofParams;
...
void print_message(char* request, char* ip){
  printf("O host %s is redirect to a %s\n", ip, request);
}
...
void usage(char *prog_name){
  fprintf(stderr, "Usage:%s <siteaddress> <ip>\n", prog_name);
  exit(-1);
}
...
void parse_args(int argc, char *argv[], SpoofParams *spoof_params){
  ...
  if(argc != 3){
    ...
    scanf("%d" , &n);
    devname = devs[n];
    printf("selected port is : %s",devname);
  
    
      strncpy(spoof_params->interface, devname, PCAP_INTERFACENAME_SIZE-1);
      spoof_params->interface[PCAP_INTERFACENAME_SIZE-1] = '\0';
   
    
  
      strncpy(spoof_params->request, argv[1], REQUEST_SIZE-1);
      spoof_params->request[REQUEST_SIZE-1] = '\0';
    
    
    
      strncpy(spoof_params->ip, argv[2], IP_SIZE-1);
